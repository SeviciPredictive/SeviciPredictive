...
/*
   ...
   public static List<Station> parseArrayFromJson(JSONArray jsonArray) {
        ...
        for (int ii = 0; ii < jsonArray.length(); ++ii) {
            try {
                ...
                String name = stationJson.getString(NAME_KEY);
                String address = stationJson.getString(ADDRESS_KEY);
                String status = stationJson.getString(STATUS_KEY);
                int availableBikes = stationJson.getInt(AVAILABLE_BIKES_KEY);
                int totalBikeStands = stationJson.getInt(TOTAL_STANDS_KEY);

                // Process the strings so they're prettie
